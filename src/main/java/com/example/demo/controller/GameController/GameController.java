...
import com.example.demo.model.response.PlayerResponse;
import com.example.demo.model.response.SpaceResponse;
...
public class GameController {
    ...
    public ResponseEntity<PlayerDto> addPlayer(@PathVariable("boardId") int boardId, @RequestBody PlayerDto playerDto) throws ServiceException, MappingException, DaoException {
        Board board = gameService.getBoard(boardId);
        Player player = dtoMapper.convertToEntity(playerDto, board);
        if(player == null){
            player = gameService.getPlayer(playerDto.getPlayerId());
        }
        player.board = board;
        int playerId = gameService.addPlayer(boardId, player);
        Space s = player.getSpace();
        Integer x = s == null ? 0 : s.x;
        Integer y = s == null ? 0 : s.y;
        PlayerDto playerDtoResponse = new PlayerDto();
        playerDtoResponse.setPlayerId(playerId);
        playerDtoResponse.setBoardId(boardId);
        playerDtoResponse.setPlayerName(player.getName());
        playerDtoResponse.setPlayerColor(player.getColor());
        playerDtoResponse.setX(x);
        playerDtoResponse.setX(y);
    ...
    /**
     * Endpoint for getting all existing boards
     *
     * @return boards
     */
    @GetMapping("/board")
    public ResponseEntity<BoardResponse[]> getBoards() throws ServiceException, DaoException {
        Collection<Board> boards = gameService.getBoards();
        BoardResponse[] boardResponses = new BoardResponse[boards.size()];
        Iterator<Board> iterator = boards.iterator();
        int index = 0;
        while (iterator.hasNext()) {
            Board board = iterator.next();
            int boardId = board.getGameId();

            Space[][] spaces = board.getSpaces();
            SpaceResponse[][] spaceResponses = new SpaceResponse[spaces.length][];
            for (int i = 0; i < spaces.length; i++) {
                int length = spaces[i].length;
                spaceResponses[i] = new SpaceResponse[length];
                for (int j = 0; j < length; j++) {
                    Space s = spaces[i][j];
                    Player p = s.getPlayer();
                    Integer playerId = p == null ? null : p.getPlayerId();
                    spaceResponses[i][j] = new SpaceResponse(s.x, s.y, playerId);
                }
            }

            List<Player> players = board.getPlayers();
            PlayerResponse[] playerResponses = new PlayerResponse[players.size()];
            for (int i = 0; i < players.size(); i++) {
                ...
                playerResponses[i] = new PlayerResponse(boardId, p.getPlayerId(), p.getName(), x, y);
            }

            boardResponses[index++] = new BoardResponse(
                    boardId, board.boardName, board.width, board.height, spaceResponses, playerResponses);
        }
